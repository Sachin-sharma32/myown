...
const Categories = () => {
  ...
  const [selectedCategoriesForDelete, setSelectedCategoriesForDelete] =
    useState("");
  const { data } = useGetCategories(activePage, limit);
  ...
  const addCategory = (e) => {
    ...
    if (!user && !user?.isAdmin) {
      ...
      dispatch(setMessage("Only admin can create category"));
    } else {
  ...
  const { mutate: createCategory } = useCreateCategory(onSuccess, onError);
  ...
  return (
    <div className="bg-[#f8f9fa] py-1 cfbp:px-20 px-4 font-gilroy5 min-h-screen">
      <Dialog
      ...
      <h1 className=" my-10 text-3xl">CATEGORIES</h1>
            ...
            <div className=" text-sm  bg-[#ffffff] rounded-xl shadow-xl">
                ...
                <div className="text-black flex gap-2 items-center">
                  ...
                  <button
                    onClick={() => {
                      if (!user && !user?.isAdmin) {
                        dispatch(setError(true));
                        dispatch(setMessage("Only admin can delete category "));
                      } else {
                    ...
                    >
                      <td className="pl-4">
                        <Checkbox
                          ...
                          onChange={(e) => {
                            if (
                              typeof selectedCategoriesForDelete === "string"
                            ) {
                              if (selectedCategoriesForDelete == category._id) {
                                setSelectedCategoriesForDelete("");
                              } else {
                                setSelectedCategoriesForDelete(category._id);
                              }
                            } else {
                              if (
                                selectedCategoriesForDelete?.includes(
                                  category._id
                                )
                              ) {
                                setSelectedCategoriesForDelete((s) => {
                                  const index =
                                    selectedCategoriesForDelete.indexOf(
                                      category._id
                                    );
                                  return selectedCategoriesForDelete.splice(
                                    index,
                                    1
                                  );
                                });
                          ...
                          checked={
                            (Array.isArray(selectedCategoriesForDelete) &&
                              selectedCategoriesForDelete.includes(
                                category._id
                              )) ||
                            (typeof selectedCategoriesForDelete === "string" &&
                              selectedCategoriesForDelete == category._id)
                          }
                        />
                      </td>
              ...
              <div className="p-10  flex items-end justify-between">
                <p className=" hidden tbp:flex text-[#fb8500]">
                  ...
                  to{" "}
                  <span className="text-black mr-1 ml-1">
                    {limit * activePage - (limit - data?.categories?.length)}
                  </span>{" "}
