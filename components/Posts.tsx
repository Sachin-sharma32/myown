...
const Posts = ({ values }: PostsProps) => {
  let user = useSelector((state: State) => state.base.user);
  ...
  const { data, isFetchingNextPage, hasNextPage, fetchNextPage, isLoading } =
    useInfiniteQuery(
      ...
      ({ pageParam = 1 }) => {
        return axios.get(
          `http://localhost:8000/api/v1/posts?page=${pageParam}&limit=12&sort=${filter}&filter=${values.filter}&type=${values.type}`
        );
      },
      {
      ...
      <section className=" p-10 md:w-[100%] flex flex-col justify-center items-center gap-2 md:gap-10">
        ...
        >
          {data?.pages.map((group, i) => (
            <Fragment key={i}>
              {group.data.data.docs.map((post: PostProp, index: number) => (
