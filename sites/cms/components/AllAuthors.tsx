...
const Users = () => {
  ...
  const { mutate: deletePost } = useDeleteUser();
  const [editorState, setEditorState] = React.useState(null);
  const [limit, setLimit] = useState(5);
  const filter = useSelector((state: State) => state.base.filter);
  const [activePage, setActivePage] = useState(1);
  const { data } = useGetAuthors(activePage, limit, filter);
  data;
  ...
  const handleDelete = () => {
    deletePost(selectedUserForDelete);
              ...
              <button
                onClick={() => {
                  if (!user && !user?.isAdmin) {
                    ...
                    dispatch(setMessage("Only admin can remove himself"));
                  } else {
                    ...
                    <Checkbox
                      ...
                      onChange={(e) => {
                        if (typeof selectedUserForDelete === "string") {
                          if (selectedUserForDelete == user._id) {
                            setSelectedUserForDelete("");
                          } else {
                            setSelectedUserForDelete(user._id);
                          }
                        } else {
                          if (selectedUserForDelete?.includes(user._id)) {
                            ...
                            setSelectedUserForDelete((s) => {
                              if (Array.isArray(s)) {
                                const updatedArray = [...s]; 
                                updatedArray.push(user._id); 
                                return updatedArray;
                              }
                            });
                          }
                      ...
                      checked={
                        (Array.isArray(selectedUserForDelete) &&
                          selectedUserForDelete.includes(user._id)) ||
                        (typeof selectedUserForDelete === "string" &&
                          selectedUserForDelete == user._id)
                      }
                    />
                  </td>
                  <td className=" py-4">
